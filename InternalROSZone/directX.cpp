...
//void CDraw::Text(float x, float y, int orientation, int font, bool bordered, DWORD color, DWORD bcolor, const char *text)
//{
//	RECT rect;
//
//	switch (orientation)
...
void CDraw::Message(float x, float y, int font, int orientation, const char *text)
{
	RECT rect = { x, y, x, y };
	...
	{
	case lefted:
		pFont[font]->DrawTextA(NULL, text, -1, &rect, DT_CALCRECT | DT_LEFT, BLACK);

		BoxRounded(x - 5, rect.top - 5, rect.right - x + 10, rect.bottom - rect.top + 10, 5, true, DARKGRAY, SKYBLUE);

		SetRect(&rect, x, y, x, y);
		pFont[font]->DrawTextA(NULL, text, -1, &rect, DT_LEFT | DT_NOCLIP, ORANGE);
		break;
	case centered:
		pFont[font]->DrawTextA(NULL, text, -1, &rect, DT_CALCRECT | DT_CENTER, BLACK);

		BoxRounded(rect.left - 5, rect.top - 5, rect.right - rect.left + 10, rect.bottom - rect.top + 10, 5, true, DARKGRAY, SKYBLUE);

		SetRect(&rect, x, y, x, y);
		pFont[font]->DrawTextA(NULL, text, -1, &rect, DT_CENTER | DT_NOCLIP, ORANGE);
		break;
	case righted:
		pFont[font]->DrawTextA(NULL, text, -1, &rect, DT_CALCRECT | DT_RIGHT, BLACK);

		BoxRounded(rect.left - 5, rect.top - 5, rect.right - rect.left + 10, rect.bottom - rect.top + 10, 5, true, DARKGRAY, SKYBLUE);

		SetRect(&rect, x, y, x, y);
		pFont[font]->DrawTextA(NULL, text, -1, &rect, DT_RIGHT | DT_NOCLIP, ORANGE);
		break;
	}
}

void CDraw::ESPBox(int x, int y, int w, int h, int thickness, D3DCOLOR Colour)
{
	FilledRect(x, y, w, thickness, Colour);
	//Left vertical line
	FilledRect(x, y, thickness, h, Colour);
	//right vertical line
	FilledRect((x + w), y, thickness, h, Colour);
	//bottom horiz line
	FilledRect(x, y + h, w + thickness, thickness, Colour);
}

void CDraw::FilledRect(int x, int y, int w, int h, D3DCOLOR color)
{
	D3DRECT BarRect = { x, y, x + w, y + h };
	//We clear that portion of the screen and display our rectangle
	this->pDevice->Clear(1, &BarRect, D3DCLEAR_TARGET | D3DCLEAR_TARGET, color, 0, 0);
}

void CDraw::Text(int x, int y, D3DCOLOR color, int font, const char* text, ...)
{
	char buffer[256];
	va_list args;
	va_start(args, text);
	...
	RECT rct;
	SetRect(&rct, x, y, x, y);
	pFont[font]->DrawText(NULL, buffer, -1, &rct, DT_NOCLIP, color);
}

void CDraw::MyFilledRect(int x, int y, int w, int h, D3DCOLOR color)
{
	D3DRECT BarRect = { x, y, x + w, y + h }; //create the rect
	pDevice->Clear(1, &BarRect, D3DCLEAR_TARGET | D3DCLEAR_TARGET, color, 0, 0); //clear = re-draw target area
}

void CDraw::BorderedRect(int x, int y, int w, int h, int thickness, D3DCOLOR colorFilled, D3DCOLOR colorBordered)
{
	MyFilledRect(x, y, w, h, colorFilled);
	ESPBox(x, y, w, h, thickness, colorBordered);
}
